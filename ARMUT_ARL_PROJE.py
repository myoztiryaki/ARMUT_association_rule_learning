...
df_ = pd.read_csv("odev_recommender_systems/ARMUTARL/armut_data.csv")
df = df_.copy()
df.head()
df.shape
df.describe().T
...
df["Hizmet"] = df[["ServiceId", "CategoryId"]].astype(str).agg(lambda x: "_".join(x), axis=1)
df.head()
...
df["SepetID"] = df[["UserId", "New_Date"]].astype(str).agg(lambda x: "_".join(x), axis=1)
df.head()
